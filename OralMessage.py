...
def OralMessage(generals, commander, M, initialValues, isFaulty, indent):
    ...
    PrintIndent(indent)
    print 'OM(%d)' % (M), 'commander: %d,' % (commander), 'generals:', generals
    ...
    if M == 0:
	    ...
	    result.append(1 if cntOfOnes > cntOfZeros else 0)
    ...
    for i in range(0, len(result)):
        PrintIndent(indent)
        print 'General %d confirms \'%s\'.' % (generals[i], 'attack' if result[i] == 1 else 'retreat') \
              if not isFaulty[generals[i]] else 'General %d is faulty.' % (generals[i])
